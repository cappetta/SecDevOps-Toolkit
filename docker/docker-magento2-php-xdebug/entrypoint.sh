...
if [ -n "${COMMAND}" ]; then
  ...
  CMD_CONFIG="${CMD_MAGENTO} setup:config:set --db-host="${MARIADB_HOSTNAME}" \
              --db-name="${MARIADB_DATABASE}" --db-user="${MARIADB_USERNAME}" \
              --db-password="${MARIADB_PASSWORD}" --key="${CRYPTO_KEY}""
